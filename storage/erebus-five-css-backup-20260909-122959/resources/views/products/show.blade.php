...
<link rel="stylesheet" href="{{ asset('css/erebus/views/products/show.css') }}">
@section('content')
        ...
        <div class="products-show-main">
            ...
            <div class="products-show-wishlist">
                @if(Auth::user()->hasWishlisted($product->id))
                    <form action="{{ route('wishlist.destroy', $product) }}" method="POST inline-048b9a2b26">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="products-show-wishlist-button products-show-wishlist-remove">❤️ Remove from Wishlist</button>
                    </form>
                @else
                    <form action="{{ route('wishlist.store', $product) }}" method="POST inline-048b9a2b26">
                        @csrf
